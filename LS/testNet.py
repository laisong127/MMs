...
def saveresult(test_loader, model):
    ...
    for img, label, _, labelfile in test_loader:
        ...
        nib.save(new_image, '../save_result/pred_{}'.format(labelfile[0]))

        # new_label = nib.Nifti1Image(label, np.eye(4))
        # nib.save(new_label, r'../save_result/label_%d.nii.gz' % i)
...
def test_model():
    ...
    model.load_state_dict(torch.load('../LS/3dunet_model_save/weights_149.pth'))
...
if __name__ == '__main__':
    ...
    # test_loader = DataLoader(test_dataset, batch_size=1, shuffle=False, num_workers=4)
    # model = Insensee_3Dunet(1).to(device)
    # model.load_state_dict(torch.load(r'../LS/3dunet_model_save/weights_149.pth'))
    # saveresult(test_loader, model)
